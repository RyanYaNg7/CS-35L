  ...
  for( int px = 0, pos = 0, row_size = height * 3; px < width; ++px )
    {
	  ...
	  for( int i = 0; i < 3; i++)
	    scaledArray[pos + i] = max( min(scaled_color[i], 255), 0);
        }
    }
  return NULL;
}
...
int main( int argc, char **argv )
{
  ...
  scaledArray = (float*) malloc(sizeof(float) * size);
  pthread_t*  tid = (pthread_t*) malloc(sizeof(pthread_t) * nthreads);
  Argument_t* argument = (Argument_t*) malloc(sizeof(Argument_t) * nthreads);
  if (tid == NULL || argument == NULL || scaledArray == NULL)
  ...
  while (pos < size)
    {
       ...
       */
      printf( "%.0f %.0f %.0f\n",
	      scaledArray[pos], scaledArray[pos+1], scaledArray[pos+2] );
      pos += 3;
      if (pos % row_size == 0)
	printf("\n");
    }
